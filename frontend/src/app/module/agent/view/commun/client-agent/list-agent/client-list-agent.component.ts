...
@Component({
  selector: 'app-client-list-agent',
  templateUrl: './client-list-agent.component.html'
})
export class ClientListAgentComponent extends AbstractListController<ClientDto, ClientCriteria, ClientService>  implements OnInit {
...
, private clientCategoryService: ClientCategoryService) {
    ...
    public async loadClientCategory(){
        await this.roleService.findAll();
        const isPermistted = await this.roleService.isPermitted('Client', 'list');
        isPermistted ? this.clientCategoryService.findAll().subscribe(clientCategorys => this.clientCategorys = clientCategorys,error=>console.log(error))
